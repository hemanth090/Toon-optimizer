...
def query_data_with_gemini_util(data_text: str, question: str, data_format: str):
    ...
    # Create prompt - TOON is for INPUT efficiency, LLMs output JSON naturally
    # Research confirms: TOON input = 30-70% token savings, output = JSON (natural)
...
Provide a clear, natural language answer. Be direct and concise."""
    ...
    # Count output tokens in both formats for comparison
    output_json_tokens = 0
    output_toon_tokens = 0
    output_conversion_status = "not_applicable"
    
    # Try to detect if answer contains structured data and convert it
    try:
        # If answer contains JSON-like structure, try to convert
        if "{" in answer or "[" in answer:
            # Try to extract and convert JSON from answer
            import re
            ...
            if json_match:
                ...
                # Count as JSON
                output_json_tokens = count_tokens(json_str)
                
                # Convert to TOON and count
    ...
    return {
        ...
        "output_json_tokens": output_json_tokens,
        "output_toon_tokens": output_toon_tokens,
